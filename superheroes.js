...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    for (let superhero of superheroes) {
        superheroCard += `<div class="hero-card">
        <h2>${superhero.name}</h2>
        <div class="hero-pic"><img src="${superhero.pic}" alt="${superhero.alt}"></div>
        <div class="hero-info">
        <p><span class="sub-title">Вселенная: </span>${superhero.universe}</p>
        <p><span class="sub-title">Альтер-эго: </span>${superhero.alterEgo}</p>
        <p><span class="sub-title">Род деятельности: </span>${superhero.occupation}</p>
        <p><span class="sub-title">Друзья: </span>${superhero.friends}</p>
        <p><span class="sub-title">Суперсилы: </span>${superhero.powers}</p><br><br>
        </div>
        <div class="hero-descr">
        ${superhero.description}
        <form onchange="rateHero(${i});">
        <p class="sub-title">Оцените героя:</p>
        <span class="checkbox-rate">
        <input name="rate${i}" id="rate1${i}" type="radio" value="1"><label for="rate1${i}"></label>
        </span>

        <span class="checkbox-rate">
        <input name="rate${i}" id="rate2${i}" type="radio" value="2"><label for="rate2${i}"></label>
        </span>

        <span class="checkbox-rate">
        <input name="rate${i}" id="rate3${i}" type="radio" value="3"><label for="rate3${i}"></label>
        </span>

        <span class="checkbox-rate">
        <input name="rate${i}" id="rate4${i}" type="radio" value="4"><label for="rate4${i}"></label>
        </span>

        <span class="checkbox-rate">
        <input name="rate${i}" id="rate5${i}" type="radio" value="5"><label for="rate5${i}"></label>
        </span>
        </form>

        <div id="result${i}">` + `${(superhero.rate ? `Ваша оценка:<br>${superhero.rate}` : "Ваша оценка")}` + `</div>
        </div>
        </div>`;
        i++;
    }
...
function rateHero(id) {
    let rates = document.getElementsByName('rate' + id);
    let rate;
    for (let r of rates) {
        if (r.checked) {
            rate = r.value;
            document.getElementById('result' + id).innerHTML = `Ваша оценка:<br>${rate}`;
        }
    }
    console.log(rate);
    console.log(document.getElementById('result' + id));

    superheroes[id].rate = rate; //меняется изначальный массив
    localStorage.setItem('superheroes', JSON.stringify(superheroes));
}

//<button id="hero-btn" type="button" onclick="rateHero(${i});">оценить</button>